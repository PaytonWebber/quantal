...
Goal: settle the scaling claim — quantajump's ~O(log n) routing vs turbovec's
...
- Time estimates: conversion ~10 min; quantajump build ~60-120 s (12T) +
  ground truth ~15-20 min (computed once per qj-bench invocation) + searches;
...
- build time, resident memory (qj-bench prints its breakdown)
...
- quantajump per-query cost grows ~1.5-2.5x (deeper graph, larger beam to
